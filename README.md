...
**Series cards** (home page carousel) show each series' own completion — "3/5" or "COMPLETED" — rolled up from the same `ttl-progress` data the episode router writes. Every `markComplete()` call on a series' own episode page now also records `completedCount`/`total` (not just a `completed` boolean once it hits 100%) against that series' id, so a home page card can read `${completedCount}/${total}` directly without any extra fetch or per-series episode list of its own — just the same Custom Field, read local-first then re-rendered once Memberstack hydrates. `.ttlr_series_card-inner` (if present) is sized to that same percentage as a mini fill bar, same "only control how much shows, `background` stays entirely Designer's" approach as the main progress bar.
...
<link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/elisa-sanchez-mb/sky-ttlr@v1.6.2/sky-ttlr.css">
```

Add to Webflow Site Settings → Custom Code → **before `</body>`** (after `webflow.js` and after Memberstack's own script tag):

```html
<script src="https://cdn.jsdelivr.net/gh/elisa-sanchez-mb/sky-ttlr@v1.6.2/sky-ttlr.js"></script>
```
...
- **`data-series-id`** on `.ttlr_series_card-wrap` (the home page carousel card's own link element) — bind to that Series item's own Item ID or Slug, and it **must match exactly** the `data-series-id` used on that same series' episode-list page above, or its completion never rolls up to this card.
- **`[data-series-card="status"]`** — a text element inside `.ttlr_series_card-content` (or wherever the design calls for it) — the script overwrites its text with `"3/5"` or `"COMPLETED"`. Left untouched (whatever static text Designer put there) until the member has actually recorded progress on that series.
- **`.ttlr_series_card-inner`** (already present in the current markup, nested in `.ttlr_series_card-progress`) — optional; if present, its `width` is set to match the same completion %. Nothing to add here, just noting it's read.
